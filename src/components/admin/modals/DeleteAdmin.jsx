...
import { deleteAdmin } from "../../../redux/teamSlice";

export default function DeleteAdmin({ id }) {
  let [isOpen, setIsOpen] = useState(false);
  const { token } = useSelector((state) => state.auth);
  const { loadingDelete } = useSelector((state) => state.team);
  ...
  return (
    <>
      <button
        onClick={openModal}
        className="bg-red-700 w-[160px] rounded-[8px] py-2 text-[16px] font-bold text-white"
      >
        Delete
      </button>
